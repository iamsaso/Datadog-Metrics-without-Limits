...
func main() {
	...
	for _, metric := range resp.GetUsage() {
		...
		fmt.Println("Metric Type: ", *metricMetadataResp.Type)
		...
		if listTagConfigurationByNameResp.Data != nil {
			...
			updateTagConfigurationResp, r, err := metricApiV2.UpdateTagConfiguration(ctx, metricID, body)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error when calling `MetricsApi.UpdateTagConfiguration`: %v\n", err)
				fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
			}

			responseContent, _ := json.MarshalIndent(updateTagConfigurationResp, "", "  ")
			...
			createTagConfigurationnResp, r, err := metricApiV2.CreateTagConfiguration(ctx, metricID, tag)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error when calling `MetricsApi.UpdateTagConfiguration`: %v\n", err)
				fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
			}
	
			responseContent, _ := json.MarshalIndent(createTagConfigurationnResp, "", "  ")
			fmt.Fprintf(os.Stdout, "Response from `MetricsApi.UpdateTagConfiguration`:\n%s\n", responseContent)
		}
	}
}